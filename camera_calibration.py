...
# Critério de terminação: ou após 30 iterações ou se os cantos forem encontrados com a precisão desejada.
criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)

# Prepara os pontos do objeto (0,0,0), (1,0,0), (2,0,0) ...., (6,5,0)
# As dimensões (7, 10) são baseadas no número de cantos em seu tabuleiro de xadrez (número de quadrados por dimensão menos um)
objp = np.zeros((7*10, 3), np.float32)
objp[:, :2] = np.mgrid[0:10, 0:7].T.reshape(-1, 2)

# Cria listas para armazenar os pontos do objeto e os pontos da imagem de todas as imagens.
...
images = glob.glob('chessboard/IMG*.JPG')
...
for fname in images:
    ...
    # Se os cantos forem encontrados, adicione os pontos do objeto e os pontos da imagem (após refinar os cantos)
    if ret == True:
...
# Imprime os valores de calibração
